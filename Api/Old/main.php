...
<script>
  (function(){
    ...
    const SCH_API = '/Api/getScholarshipData.php';
  ...
  // helper: render consolidated scholarship card (one or many)
  async function renderScholarshipRecord(rec) {
    // rec: name, income_limit_texts, income_min, income_max, criteria, documents_required[], site_urls[], eligibility_categories[], courses[]
    const wrap = document.createElement('div'); wrap.className = 'msg ai';
    const b = document.createElement('div'); b.className = 'bubble';

    let html = `<div style="font-weight:800;margin-bottom:8px">${escapeHtml(rec.name)}</div>`;
    html += `<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">`;
    if (rec.income_limit_texts && rec.income_limit_texts.length) {
      html += `<div class="pill">${escapeHtml(rec.income_limit_texts[0])}</div>`;
    }
    if (rec.income_min) html += `<div class="pill">Min ₹${numberWithCommas(rec.income_min)}</div>`;
    if (rec.income_max) html += `<div class="pill">Max ₹${numberWithCommas(rec.income_max)}</div>`;
    html += `</div>`;

    html += `<div style="font-weight:700;margin-bottom:6px">Eligibility / Criteria</div><div class="muted" style="white-space:pre-wrap">${escapeHtml(rec.criteria || 'Not specified')}</div>`;

    html += `<div style="font-weight:700;margin-top:10px;margin-bottom:6px">Required Documents</div><div class="muted">${(Array.isArray(rec.documents_required) && rec.documents_required.length) ? rec.documents_required.map(d=>escapeHtml(d)).join(', ') : 'Not specified'}</div>`;

    html += `<div style="font-weight:700;margin-top:10px;margin-bottom:6px">Eligible Categories</div>`;
    html += `<div>${(Array.isArray(rec.eligibility_categories) && rec.eligibility_categories.length) ? rec.eligibility_categories.map(c=>`<span class="pill">${escapeHtml(c)}</span>`).join(' ') : '<span class="muted">Not specified</span>'}</div>`;

    html += `<div style="font-weight:700;margin-top:10px;margin-bottom:6px">Reference Links</div>`;
    if (Array.isArray(rec.site_urls) && rec.site_urls.length) {
      for (const u of rec.site_urls) html += `<div><a class="site-link" href="${escapeAttr(u)}" target="_blank" rel="noopener noreferrer">${escapeHtml(u)}</a></div>`;
    } else {
      html += `<div class="muted">No reference links</div>`;
    }

    // courses table
    if (Array.isArray(rec.courses) && rec.courses.length) {
      html += `<div style="font-weight:700;margin-top:12px;margin-bottom:6px">Courses & Sanctioned Amounts</div>`;
      html += `<table style="width:100%;font-size:13px"><thead><tr><th>Course/Year</th><th>Sanction (text)</th><th>Amount (min)</th></tr></thead><tbody>`;
      for (const c of rec.courses) {
        html += `<tr><td>${escapeHtml(c.course_label)}</td><td class="muted">${escapeHtml(c.scholarship_text||'')}</td><td>${c.sch_min ? '₹'+numberWithCommas(c.sch_min) : '-'}</td></tr>`;
      }
      html += `</tbody></table>`;
    }
    ...
    scrollToBottom();
    // attach feedback controls for entire record (no single result id available)
    attachFeedbackControls(b, rec.name, 'SCHOLAR');
  }

  // New sendToAPI for scholarship endpoint
  async function sendToAPI(queryText){
    ...
    try {
      const body = { auth_token: AUTH_TOKEN, query: queryText, limit: 6 };
      ...
      typingWrap && typingWrap.remove();

      if (!data || data.status !== 'ok') {
        await showTranslatedAndType('Sorry — ' + (data && data.message ? data.message : 'unable to fetch scholarships'), 20);
        return;
      }

      // If spelling suggestion present
      ...
      // If results present, render them (consolidated)
      if (Array.isArray(data.results) && data.results.length > 0) {
        // type short summary first
        const summary = `${data.results.length} result(s) found for "${data.query || queryText}"`;
        await showTranslatedAndType(summary, 22);
      ...
      // nothing found — suggest nearest name if provided
      if (data.suggested_name) {
        await showTranslatedAndType(`No exact match found. Did you mean "${data.suggested_name}"?`, 20);
        // show clickable suggestion button
        const follow = document.createElement('div'); follow.className = 'msg ai';
        const fb = document.createElement('div'); fb.className = 'bubble';
        fb.innerHTML = `<div style="display:flex;gap:8px;align-items:center"><button class="lang-cap suggestion accept-correction">${escapeHtml(data.suggested_name)}</button> <button class="lang-cap ignore-correction">No, keep "${escapeHtml(queryText)}"</button></div>`;
        follow.appendChild(fb); messages.appendChild(follow);
        scrollToBottom();
        // handlers are in message click listener below (accept-correction -> re-run)
        return;
      }

      // fallback suggestions
      if (Array.isArray(data.nearest_suggestions) && data.nearest_suggestions.length) {
        await showTranslatedAndType('Could not find exact scholarship. Try one of these:', 20);
        translateSuggestionsAndRender(data.nearest_suggestions);
        return;
      }

      await showTranslatedAndType('Sorry, I could not find relevant scholarships for your query.', 20);

    } catch (err) {
      typingWrap && typingWrap.remove();
      console.error(err);
      await showTranslatedAndType('Sorry — an error occurred while fetching scholarships.', 20);
    } finally {
      scrollToBottom();
    }
  }
  ...
  messages && messages.addEventListener('click', function(e){
    const accept = e.target.closest('.accept-correction');
    const ignore = e.target.closest('.ignore-correction');
    if (accept) {
      const txt = accept.textContent || accept.innerText;
      sendEvent({ event:'click', result_type:'CORRECTION', query: txt });
      sendToAPI(txt);
      const wrap = accept.closest('.msg.ai'); if (wrap) wrap.remove();
      return;
    }
    if (ignore) {
      const wrap = ignore.closest('.msg.ai'); if (wrap) wrap.remove();
      return;
    }
  });
...
async function sendToAPI3(queryText){
  if (!queryText || !queryText.trim()) return;
  addBubble(queryText.trim(), 'user');
  if (input) input.value = '';
